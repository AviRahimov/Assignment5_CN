...
def spoof_tcp(src_ip, dest_ip):
    # Create a TCP packet with custom source port, destination port, flags, and window size
    tcp_packet = IP(src=src_ip, dst=dest_ip) / TCP(sport=1234, dport=5678, flags="SA", window=2000)
    # Send the packet and print the result
